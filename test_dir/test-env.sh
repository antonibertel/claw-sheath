...
bash ../../cs -c "rm a.txt --sheathJustify '!override!'"
if [[ -f a.txt ]]; then echo "FAIL 3: a.txt still exists!"; exit 1; fi

echo "[Test 4] Subshells and Chains (should fail on rm)"
if bash ../../cs -c "touch sub.txt && (ls; rm sub.txt) || echo 'failed'" 2>/dev/null; then
    :
fi
...
if bash ../../cs -c 'CMD="rm c.txt"; $CMD' 2>/dev/null; then
...
bash ../../cs -c 'CMD="rm c.txt --sheathJustify !override!"; $CMD'
...
bash ../../cs -c "rm 'space file.txt' --sheathJustify '!override!'"
...
bash ../../cs -c "kubectl delete pod my-pod --sheathJustify '!override!' > output_kubectl_del.txt"
