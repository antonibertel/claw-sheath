...
bash ../../src/sheath-shell.sh -c "rm a.txt --sheathJustify '!override!'"
if [[ -f a.txt ]]; then echo "FAIL 3: a.txt still exists!"; exit 1; fi

echo "[Test 4] Subshells and Chains (should fail on rm)"
if bash ../../src/sheath-shell.sh -c "touch sub.txt && (ls; rm sub.txt) || echo 'failed'" 2>/dev/null; then
    :
fi
...
if bash ../../src/sheath-shell.sh -c 'CMD="rm c.txt"; $CMD' 2>/dev/null; then
...
bash ../../src/sheath-shell.sh -c 'CMD="rm c.txt --sheathJustify !override!"; $CMD'
...
bash ../../src/sheath-shell.sh -c "rm 'space file.txt' --sheathJustify '!override!'"
...
if bash ../../src/sheath-shell.sh -c "kubectl delete pod my-pod" 2>/dev/null; then
    echo "FAIL 10: kubectl delete succeeded without justify!"
    exit 1
fi

echo "[Test 11] Conditional kubectl get without justify (should succeed)"
bash ../../src/sheath-shell.sh -c "kubectl get pods > output_kubectl.txt"
...
bash ../../src/sheath-shell.sh -c "kubectl delete pod my-pod --sheathJustify '!override!' > output_kubectl_del.txt"
